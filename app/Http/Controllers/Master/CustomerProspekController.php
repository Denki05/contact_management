...
<<<<<<< HEAD
    ...
    //     // Default fallback agar $customers selalu terdefinisi
    //     $customers = collect();
    ...
    //         $pengajuanList = "KANTOR";
    
    //         $customers = DB::table('master_customer_other_addresses as c')
    //             ->join('master_customers as s', 's.id', '=', 'c.customer_id')
    //             ->leftJoin('master_customer_categories as cat', 'cat.id', '=', 's.category_id')
    //             ->where('s.status', 1)
    //             ->select(
    //                 'c.*',
    ...
    //         $customers = DB::table('master_customer_other_addresses_prospek as c')
    //             ->leftJoin('master_customers_prospek as s', 's.id', '=', 'c.customer_id')
    //             ->leftJoin('master_customer_categories as cat', 'cat.id', '=', 's.category_id')
    //             ->where('s.status', 1)
    //             ->select(
    //                 'c.*',
    ...
    //                 's.website as website',
    //                 's.pic as pic',
    //                 DB::raw("'prospek' as type")
    //             )
    //             ->orderBy('c.name', 'asc')
    //             ->get()
    //             ->map(function ($item) use ($pengajuanList) {
    //                 $item->pengajuan_label = $pengajuanList[$item->pengajuan] ?? '-';
    //                 return $item;
    //             });
    
    //     }
    //     // ----------------------------------------
    //     // 3. ALL (GABUNG EXISTING + PROSPEK)
    //     // ----------------------------------------
    //     elseif ($type === 'all') {
    ...
    //         $existing = DB::table('master_customer_other_addresses as c')
    //             ->join('master_customers as s', 's.id', '=', 'c.customer_id')
    //             ->leftJoin('master_customer_categories as cat', 'cat.id', '=', 's.category_id')
    //             ->where('s.status', 1)
    //             ->select(
    ...
    //                 's.pic as pic',
    //                 DB::raw("'existing' as type"),
    //                 DB::raw("null as pengajuan_label")
    //             )
    //             ->get()
    //             ->map(function ($item) use ($pengajuanList) {
    //                 $item->pengajuan_label = $pengajuanList[$item->pengajuan] ?? 'KANTOR';
    //                 return $item;
    //             });
    ...
    //         $prospek = DB::table('master_customer_other_addresses_prospek as c')
    //             ->leftJoin('master_customers_prospek as s', 's.id', '=', 'c.customer_id')
    //             ->leftJoin('master_customer_categories as cat', 'cat.id', '=', 's.category_id')
    //             ->select(
    //                 'c.*',
    ...
    public function partial(Request $request)
    {
        ...
        // KATEGORI
        // -------------------------
        $kategori = DB::table('master_customer_categories')->get();
        ...
        $slicedCustomers = $customers->forPage($page, $perPage)->values();

        // -------------------------
        // RETURN PARTIAL VIEW
        // -------------------------
        return view('master.customer_prospek.partial_index', [
            'customers'    => $slicedCustomers,
            'kategori'     => $kategori,
            'data_type'    => $view_data_type,
            'current_page' => $page,
    ...
    public function partialCreate()
    {
        ...
        return view('master.customer_prospek.partial_create', $data);
    }
    ...
    public function updateExisting(Request $request, $id)
    {
       ...
       $request->validate([
            ...
            'text_kota'         => 'nullable|string|max:100',
            'text_provinsi'     => 'nullable|string|max:100',
            'website'           => 'nullable|string|max:255',
            'pic'               => 'nullable|string|max:100',
            
            'category_id'       => 'nullable|numeric', // ← dari integer ke numeric
            'parent_id'         => 'required|numeric', // ← dari integer ke numeric
        ]);
    
        try {
            // ── 1. Update child table: master_customer_other_addresses ──
            ...
            // ── 2. Update parent table: master_customers (PIC & Kategori) ──
            ...
            return response()->json([
                'success' => true,
                'message' => 'Data existing berhasil diperbarui.',
            ]);
    ...
    public function updateProspek(Request $request, $id)
    {
        if (!auth()->check()) {
            abort(403, 'Unauthorized access');
        }
        ...
        try {
            // ── 1. Update child table: master_customer_other_addresses_prospek ──
            DB::table('master_customer_other_addresses_prospek')
                ->where('id', $id)
                ->update([
                    'name'       => $request->name,
            ...
            // ── 2. Update parent table: master_customers_prospek (PIC saja) ──
            DB::table('master_customers_prospek')
                ->where('id', $request->parent_id)
                ->update([
                    'pic'        => $request->pic,
                    'updated_at' => now(),
    ...
    {
        // Sesuaikan dengan nama Model Provinsi Anda
        $provinces = \App\Models\Province::select('prov_id', 'prov_name')->get();
        return response()->json($provinces);
    }

    public function getkabupaten(Request $request)
    {
        $prov_id = $request->prov_id; // sesuai data yang dikirim JS
    
        $kabupatens = DB::table('kabupaten')
            ->where('prov_id', $prov_id)
            ->orderBy('city_name')
            ->get(['city_id as id', 'city_name as name']);
    ...
    public function store(Request $request)
    {
        try {
            ...
            $request->validate([
                ...
                'text_provinsi' => 'nullable|string',
                'text_kota' => 'nullable|string',
            ...
            foreach ($existingProspekNames as $existingName) {
                if (levenshtein($nameToCheck, strtolower(trim($existingName))) <= $threshold) {
                    $isDuplicate = true;
            ...
            // ===================== SIMPAN STORE PROSPEK =====================
            $storeProspek = new \App\Master\StoreProspek();
            ...
            $storeProspek->pic = $request->pic;
            $storeProspek->status = \App\Master\StoreProspek::STATUS['ACTIVE'];
            $storeProspek->existence = 1;
            $storeProspek->count_member = 1;
    
            if ($request->hasFile('image_store')) {
                $storeProspek->image_store = $request->file('image_store')->store('public/images/prospek');
            }
    
            $storeProspek->save();
    
            // ===================== SIMPAN CUSTOMER PROSPEK =====================
            $customerProspek = new \App\Master\CustomerProspek();
            ...
            $customerProspek->officer = $request->officer;
            $customerProspek->member_default = 1;
            $customerProspek->status = \App\Master\CustomerProspek::STATUS['ACTIVE'];
    
            $customerProspek->save();
    
            // ===================== RESPONSE AJAX SUKSES =====================
            return response()->json([
                'message' => 'Data berhasil disimpan.'
            ]);
    
        } catch (\Illuminate\Validation\ValidationException $e) {
            // Response JSON untuk validation error
            return response()->json([
                'errors' => $e->validator->errors()
            ], 422);
    
        } catch (\Exception $e) {
            \Log::error('Gagal menyimpan data customer prospek: ' . $e->getMessage());
            return response()->json([
...
<<<<<<< HEAD
    ...
    $existing = DB::table('master_customer_other_addresses as s')
        ...
        ->select(
            ...
            's.name',
            's.officer',
            DB::raw('"KANTOR" as pengajuan_text'),
            DB::raw('UPPER(TRIM(s.zone)) as sortable_zone'),
            DB::raw('UPPER(TRIM(s.text_provinsi)) as text_provinsi'),
            DB::raw('UPPER(TRIM(s.text_kota)) as text_kota'),
            DB::raw('COALESCE(cat.name, "Tanpa Kategori") as category_name'),
            'c.pic'
        )
        ->get();
    ...
    // Gabungkan semua customer
    $customers = $existing->concat($prospek);

    // Normalisasi jika ada field kosong
    ...
    // Sorting berdasarkan category_name ASC lalu name ASC
    $customers = $customers->sortBy(function($item) {
        return $item->category_name . '|' . $item->name;
    });

    // Grouping berdasarkan zone -> provinsi -> kota
    ...
    // Urutkan zona sesuai label
    $sortedCustomers = collect();
    foreach(array_keys($zoneLabels) as $zone) {
        if($groupedCustomers->has($zone)) {
            $sortedCustomers->put($zone, $groupedCustomers->get($zone));
        }
    }

    // Tambahkan zona lain
    foreach($groupedCustomers as $zone => $data){
        if(!$sortedCustomers->has($zone)) {
            $sortedCustomers->put($zone, $data);
        }
    }
    ...
    $pdf = PDF::loadView('master.customer_prospek.pdf_report', [
        ...
        'title' => 'Laporan Customer Berdasarkan Zona, Provinsi, Kota, dan Mapping'
    ]);
    ...
    public function exportProspekPdf()
    {
        // Ambil data Prospek
        $prospek = CustomerProspek::with(['store_prospek.category'])
            ->whereHas('store_prospek', function($q) {
                $q->where('status', 1);
            })
            ->select('*', DB::raw("COALESCE(zone, 'ZONA LAIN') as sortable_zone"))
            ->orderBy('text_provinsi')
